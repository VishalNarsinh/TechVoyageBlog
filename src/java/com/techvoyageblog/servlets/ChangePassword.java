...
public class ChangePassword extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            session.removeAttribute("OTPForPasswordUpdate");
            UserDAO dao = new UserDAO(ConnectionProvider.getConnection());
            Message msg = new Message();
            session.setAttribute("msgUserSide", msg);
            if(dao.updatePasswordByEmail(email, password)){
                msg.setContent("Password updated.Please use new password to Login.");
                msg.setCssClass("alert-success");
                response.sendRedirect("login_page.jsp");
            }
            else{
                msg.setContent("Can't update Password.Please use old password to Login.");
